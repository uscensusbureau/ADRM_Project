...
def Address_Parser(Address_4CAF50,TruthSet):
  ...
  "20":"USAD_HNM"
    ...
    #                         else:
    #                             y_predict.append(0)
    with open("PuertoRicoTest.txt", 'r+', encoding='utf-8') as g:
        Stat = json.load(g)
        Count_of_Correct=0
        Total_Count=0
        ID=1
        for k in Stat["annotations"]:
            ...
            for m in k[1].items():
                print(ID)
                if str(ID) in Truth_Result.keys():
                    ...
                    for j in m[1]:
                        ...
                        for k1,v1 in Truth_Result[str(ID)].items():
                            if v1.strip().upper() == k[0][j[0]:j[1]].upper().strip():
...
Address_Parser("PuertoRico.txt","PuertoRicoTest.txt")
