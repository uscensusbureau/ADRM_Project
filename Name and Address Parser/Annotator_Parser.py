...
# if Multi_option1:
#     FileName=Multi_file_dict[Multi_option1]
...
i=1
List_Index_Mask={}
Lines=open("AddressListRev.txt","r")
...
for line in tqdm(Lines):
    ...
    for A in AddressList:
        ...
        LoopCheck+=1
    Mask_1=",".join(Mask)
    ExcceptionLikeFile[ID]=FirstPhaseList
    C+=1


FirstPhaseLisCopyt=FirstPhaseList
...
Break=False
print()
for Key,Value in list(ExcceptionLikeFile.items()):
    MaskI=1
    ...
    for k in Value:
        ...
        for v in k.values():
            val=v
        USAD_Mapping[USAD_Conversion_Dict[temp]].append(val)
        MaskI+=1
    ...
    if Break:
        break
    with open('Annotated_Data.txt', 'r+', encoding='utf-8') as f:
        data = json.load(f)
